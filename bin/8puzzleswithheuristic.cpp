...
int bfs(Node *initialNode, int* finalState) { //Busca em largura com fila
	...
	cout << "BFS -> Solução não encontrada ";
	return 0;
}
...
int dfs(Node *initialNode, int* finalState) { //busca em profundidade com pilha
	...
	while(!pilha.empty()) {
		...
		for(int i = 0; i < filhos.size(); i++) {
			if(compareStates(filhos.at(i)->state, finalState)) {
				//printState(filhos.at(i)->state); 
				cout << "DFS -> Solução encontrada com " << filhos.at(i)->cost << " Movimentos ";
				return 1;
			}else{
	...
	cout << "DFS-> Solução não encontrada ";
	return 0;
}
...
int main(void) {
	...
	*/
	...
	cout << "(" << (clock()-tempo) * 1000/CLOCKS_PER_SEC << " ms)" << endl;
	tempo = clock();
